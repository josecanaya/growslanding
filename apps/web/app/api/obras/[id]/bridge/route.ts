...
const bodySchema=z.object({action:z.enum(['pair','enqueue','cancel','apply','revoke']),jobId:z.string().uuid().optional(),deviceId:z.string().uuid().optional(),prompt:z.string().min(1).max(8000).optional(),scopePathIds:z.array(z.string().max(100)).max(50).optional(),selectionIds:z.array(z.string().max(100)).max(100).optional()});
...
export async function GET(_request:NextRequest,{params}:{params:Promise<{id:string}>}) {
 try {
  ...
  const [devices,jobs]=await Promise.all([db.from('grows_bridge_devices').select('id,last_seen_at,created_at').eq('obra_id',id).is('revoked_at',null),db.from('grows_bridge_jobs').select('id,prompt,status,result,error,created_at,updated_at,context').eq('obra_id',id).order('created_at',{ascending:false}).limit(30)]);
...
export async function POST(request:NextRequest,{params}:{params:Promise<{id:string}>}) {
 try {
  ...
  if(input.action==='enqueue') {
   ...
   const result=await db.from('grows_bridge_jobs').insert({obra_id:id,org_id:orgId,user_id:user.id,prompt:input.prompt.trim(),context:{revision:snapshot.data.revision,scopePathIds:input.scopePathIds??[],selectionIds:input.selectionIds??[]}}).select('id').single();
