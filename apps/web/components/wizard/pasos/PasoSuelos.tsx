...
interface PasoSuelosProps {
  elementosSeleccionados: Seleccion[];
  onElementosChange: (elementos: Seleccion[]) => void;
}

export function PasoSuelos({ elementosSeleccionados, onElementosChange }: PasoSuelosProps) {
  ...
  const elementosSuelos = CATALOGO_ELEMENTOS.filter(el => el.categoria === 'Suelos / Pisos');
  const seleccionesSuelos = elementosSeleccionados.filter(s => s.subgrupo === 'Suelos / Pisos');
    ...
    <div className="min-h-full bg-gray-50">
      <div className="max-w-7xl mx-auto px-6 py-8">
        ...
        <div className="mb-8">
          ...
          {seleccionesSuelos.length > 0 && (
            <div className="bg-green-50 border border-green-200 rounded-lg p-4">
              <p className="text-green-800 text-sm">
                {seleccionesSuelos.length} elementos configurados en esta etapa.
              </p>
            </div>
          )}
        </div>
            ...
            <div key={elemento.id} className="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
              ...
              <div className="space-y-3">
                {elemento.opciones.map((opcion) => {
                  const yaSeleccionado = seleccionesSuelos.some(s => s.opcionId === `${elemento.id}-${opcion.id}`);
        ...
        {seleccionesSuelos.length > 0 && (
          <div className="mt-8 bg-white rounded-xl border border-gray-200 shadow-sm p-6">
            ...
            <div className="space-y-3">
              {seleccionesSuelos.map((seleccion) => (
