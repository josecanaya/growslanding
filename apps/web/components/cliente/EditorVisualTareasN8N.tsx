...
interface EditorVisualTareasN8NProps {
  tareas: Tarea[];
  etapa: 'estructura' | 'obra_gris' | 'terminaciones';
...
export function EditorVisualTareasN8N({ 
  ...
  const precargaLogicaPorEtapas = useCallback((tareas: Tarea[]) => {
    console.log('🔧 precargaLogicaPorEtapas - tareas entrantes:', tareas.length);
    console.log('🔍 Primera tarea:', tareas[0]);
    
    // Normalizar etapas a minúsculas y reemplazar espacios por guiones bajos
    const tareasNormalizadas = tareas.map(t => ({
      ...t,
      etapa: (t.etapa || '').toLowerCase().replace(/ /g, '_') as 'estructura' | 'obra_gris' | 'terminaciones'
    }));
    ...
    ordenEtapas.forEach((etapaActual) => {
      const tareasEtapa = tareasNormalizadas.filter(t => t.etapa === etapaActual);
      console.log(`📦 Etapa ${etapaActual}: ${tareasEtapa.length} tareas`);
    ...
    console.log('✨ Total tareas posicionadas:', tareasConPosicion.length);
  ...
  useEffect(() => {
    ...
    if (tareasIniciales.length > 0) {
      ...
      console.log('✅ Nodos posicionados:', nodosConPosicion.length);
      setNodos(nodosConPosicion);
    }
  }, [tareasIniciales, precargaLogicaPorEtapas]);
  ...
  // Abrir modal de edición con doble click
  const handleDoubleClickNodo = (tarea: Tarea) => {
    if (!isDragging) {
