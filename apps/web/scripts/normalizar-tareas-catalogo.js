...
// Leer tareas-construccion.ts y extraer el array
let tareasConstructivas = [];
try {
  const tareasFile = fs.readFileSync(TAREAS_CONSTRUCCION_PATH, 'utf-8');
  // Extraer el array usando regex (buscando desde "export const tareasConstructivas")
  const arrayMatch = tareasFile.match(/export const tareasConstructivas[^=]*=\s*(\[[\s\S]*?\]);/);
  if (arrayMatch) {
    // Evaluar el array (cuidado con seguridad, pero es un archivo local)
    tareasConstructivas = eval(arrayMatch[1]);
  }
} catch (error) {
