...
const crearCreditoSchema = z.object({
 ...
 * Crea un movimiento tipo CREDITO en wallet_movimientos
 * Actualiza automáticamente el saldo en wallet_saldos
 */
export async function POST(request: NextRequest) {
  try {
    const body = await request.json();
    const payload = crearCreditoSchema.parse(body);
    ...
    if (movimientoError) {
      console.error('[WALLET_CREDITOS] Error creando movimiento:', movimientoError);
      return NextResponse.json(
        {
    ...
    if (!saldoData || saldoError) {
      ...
      if (insertError) {
        console.error('[WALLET_CREDITOS] Error creando saldo:', insertError);
    ...
    // Actualizar saldo
    if (saldoData) {
      const nuevoSaldo = (saldoData.saldo_actual ?? 0) + payload.monto;
      ...
      if (updateError) {
        console.error('[WALLET_CREDITOS] Error actualizando saldo:', updateError);
        // Continuar aunque falle, el movimiento ya se creó
      }
    }
    ...
    console.error('[WALLET_CREDITOS] Error:', error);
