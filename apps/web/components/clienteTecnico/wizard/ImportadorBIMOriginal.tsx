...
import { IfcElemento } from '../../lib/services/ifcReader';
import IFCProcessor from './IFCProcessor';

interface ImportadorBIMOriginalProps {
  onElementosImportados: (elementos: any[]) => void;
  onCancel: () => void;
}

// Usar la interfaz del ifcReader

export default function ImportadorBIMOriginal({ onElementosImportados, onCancel }: ImportadorBIMOriginalProps) {
  ...
  const handleElementosProcesados = (elementos: IfcElemento[]) => {
    setElementosProcesados(elementos);
    setProcesando(false);
  };

  const handleErrorProcesamiento = (error: string) => {
  ...
  const renderizarModelo3DReal = async (archivo: File) => {
    ...
    try {
      ...
      const modelo = await loader.loadAsync(url) as THREE.Group;
      ...
      cameraDistance *= 1.5; // Añadir margen
      ...
      console.error('Error al renderizar modelo 3D real:', error);
  ...
  const resaltarElemento = (elemento: IfcElemento) => {
    if (modeloRef.current && elemento.expressID) {
      ...
      modeloRef.current.traverse((child) => {
        if (child instanceof THREE.Mesh && child.userData.expressID === elemento.expressID) {
          if (!child.userData.originalColor) {
            child.userData.originalColor = child.material.color.getHex();
          }
          child.material.color.setHex(0x007bff); // Azul para resaltar
        }
      });
    }
  };
  ...
  if (procesando && archivoSeleccionado) {
    return (
      <IFCProcessor
        archivo={archivoSeleccionado}
        onElementosProcesados={handleElementosProcesados}
        onError={handleErrorProcesamiento}
      />
    );
  }
