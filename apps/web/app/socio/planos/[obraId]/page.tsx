...
function PlanosPageContent() {
  ...
  useEffect(() => {
    const cargarPlanos = async () => {
      ...
      try {
        // Cargar planos de la obra desde la tabla media
        const { data: mediaData, error } = await (supabase as any)
          .from('media')
          .select('*')
          .eq('obra_id', obraId)
        ...
        if (error) {
          console.error('[PlanosPage] Error cargando planos:', error);
          return;
        }

        if (mediaData) {
          setPlanos(mediaData.map((m: any) => ({
            id: m.id,
            nombre_archivo: m.nombre_archivo || m.path || 'Plano',
            url: m.url || m.path,
            ...
            {planos.map((plano) => (
              ...
              >
                ...
                <p className="text-xs text-gray-500">
                  {plano.categoria} • {new Date(plano.created_at).toLocaleDateString('es-AR')}
                </p>
              </div>
            ))}
          </div>
