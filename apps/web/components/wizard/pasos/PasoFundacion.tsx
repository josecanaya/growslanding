...
interface PasoFundacionProps {
  elementosSeleccionados: Seleccion[];
  onElementosChange: (elementos: Seleccion[]) => void;
}

export function PasoFundacion({ elementosSeleccionados, onElementosChange }: PasoFundacionProps) {
  ...
  const elementosFundacion = CATALOGO_ELEMENTOS.filter(el => el.categoria === 'Fundación y Estructura');
  const seleccionesFundacion = elementosSeleccionados.filter(s => s.subgrupo === 'Fundación y Estructura');
    ...
    <div className="min-h-full bg-gray-50">
      <div className="max-w-7xl mx-auto px-6 py-8">
        ...
        <div className="mb-8">
          ...
          {seleccionesFundacion.length > 0 && (
            <div className="bg-green-50 border border-green-200 rounded-lg p-4">
              <p className="text-green-800 text-sm">
                {seleccionesFundacion.length} elementos configurados en esta etapa.
              </p>
            </div>
          )}
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {elementosFundacion.map((elemento) => (
            <div key={elemento.id} className="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
              ...
              <div className="space-y-3">
                {elemento.opciones.map((opcion) => {
                  const yaSeleccionado = seleccionesFundacion.some(s => s.opcionId === `${elemento.id}-${opcion.id}`);
        ...
        {seleccionesFundacion.length > 0 && (
          <div className="mt-8 bg-white rounded-xl border border-gray-200 shadow-sm p-6">
            ...
            <div className="space-y-3">
              {seleccionesFundacion.map((seleccion) => (
        ...
        {showConfiguracion && elementoSeleccionado && (
          <ConfiguracionTecnicaModal
            ...
            subgrupo="Fundación y Estructura"
