...
export async function POST(
  ...
  try {
    ...
    if (rol !== 'SOCIO') {
      return NextResponse.json(
        { success: false, error: 'Solo el socio puede enviar bloques para validar' },
        { status: 403 },
      );
    }
    ...
    console.error('[ENVIAR_PARA_VALIDAR] Error:', error);
    ...
    if (errorMessage.includes('SOCIO_SUSPENDIDO') || errorMessage.includes('suspendido')) {
      errorCode = 'SOCIO_SUSPENDIDO';
      statusCode = 403;
    } else if (errorMessage.includes('FORBIDDEN')) {
      errorCode = 'FORBIDDEN_ACTION';
      statusCode = 403;
    } else if (errorMessage.includes('Debes cargar evidencia')) {
      errorCode = 'EVIDENCIA_FALTANTE';
      statusCode = 400;
    } else if (errorMessage.includes('debe estar en progreso')) {
