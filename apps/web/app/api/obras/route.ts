  ...
  try {
    ...
    try {
      ...
      if (normalizedRole === 'CLIENTE_TECNICO' || normalizedRole === 'CLIENTE') {
        /**
         * El rol de metadatos NO alcanza para autorizar: `user_metadata` lo puede
         * escribir el propio usuario (`supabase.auth.updateUser`). Exigimos además
         * pertenencia real a la organización, con el mismo criterio que usa
         * `GET /api/obras` (dueño, socio vinculado o líder invitado).
         */
        const allowedOrgIds = await listAccessibleOrgIds(supabase, user.id, user.email);
        if (allowedOrgIds.includes(org_id)) {
          console.log('[POST_OBRAS] Rol CLIENTE_TECNICO + pertenencia verificada, permitiendo creación');
          isAuthorized = true;
          userRole = 'owner';
        } else if (user.id) {
