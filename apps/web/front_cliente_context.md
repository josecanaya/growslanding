...
1. Obras - Gestión de obras activas
2. Tareas - Gestión y seguimiento de tareas
3. Cuadrillas - Administración de cuadrillas
4. Billetera - Pagos y recibos del cliente
5. Notificaciones - Alertas y notificaciones del sistema
6. Calendario - Vista de calendario de eventos
7. Cuenta - Configuración de perfil y cuenta

**NOTA:** Chat con GrowsBot fue eliminado de esta versión del panel cliente (MVP). La mensajería directa por obra sigue disponible en la sección Notificaciones > tab Mensajes.
...
**Componente:** ~~`components/cliente/ChatSection.tsx`~~ **ELIMINADO**  
**Ruta:** ~~`/cliente/dashboard?section=chat`~~ **ELIMINADO**  
**NOTA:** Chat con GrowsBot fue eliminado de esta versión del panel cliente (MVP). La mensajería directa por obra sigue disponible en Notificaciones > tab Mensajes.
...
**NOTA IMPORTANTE:**
- Chat con GrowsBot fue completamente eliminado del panel cliente en esta versión (MVP)
- La mensajería directa por obra sigue disponible en la sección Notificaciones > tab Mensajes (componente `ChatPorObra`)
- El componente `MensajeriaDirecta.tsx` existe pero no está integrado en el sidebar principal

**Endpoints relacionados (aún disponibles para mensajería directa):**
- `GET /api/mensajes` - Obtener mensajes directos por obra
- `POST /api/mensajes` - Enviar mensaje directo por obra

**Tablas Supabase (aún utilizadas para mensajería directa):**
...
- **Chat:** ❌ **ELIMINADO** - Chat con GrowsBot no se usará en esta versión del panel cliente (MVP)
...
- ~~`ChatSection.tsx`~~ - **ELIMINADO** - Chat con GrowsBot no se usará en esta versión del panel cliente (MVP)
