...
const schema=z.object({action:z.enum(['claim','heartbeat','complete','fail']),jobId:z.string().uuid().optional(),leaseToken:z.string().uuid().optional(),result:bridgeResultSchema.optional(),error:z.string().max(2000).optional()});
export async function POST(request:NextRequest){
 try{
  ...
  const ping=await db.from('grows_bridge_devices').update({last_seen_at:new Date().toISOString()}).eq('id',device.id);if(ping.error)throw ping.error;
  if(input.action==='claim'){
   ...
   return NextResponse.json({job:{id:job.id,prompt:job.prompt,canvas:supabaseRowsToPersisted(snapshot.data),scopePathIds:job.context.scopePathIds??[],selectionIds:job.context.selectionIds??[]},leaseToken:job.lease_token});
  }
  if(!input.jobId||!input.leaseToken)return NextResponse.json({message:'Falta trabajo o reserva'},{status:400});
  const patch=input.action==='heartbeat'?{lease_until:new Date(Date.now()+600000).toISOString()}:input.action==='complete'?{status:'completed',result:input.result}: {status:'failed',error:input.error??'El agente no pudo completar el trabajo.'};
