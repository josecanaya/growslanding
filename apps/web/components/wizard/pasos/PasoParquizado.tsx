...
interface PasoParquizadoProps {
  elementosSeleccionados: Seleccion[];
  onElementosChange: (elementos: Seleccion[]) => void;
}

export function PasoParquizado({ elementosSeleccionados, onElementosChange }: PasoParquizadoProps) {
  ...
  const elementosParquizado = CATALOGO_ELEMENTOS.filter(el => el.categoria === 'Parquizado');
  const seleccionesParquizado = elementosSeleccionados.filter(s => s.subgrupo === 'Parquizado');
    ...
    <div className="min-h-full bg-gray-50">
      <div className="max-w-7xl mx-auto px-6 py-8">
        ...
        <div className="mb-8">
          ...
          {seleccionesParquizado.length > 0 && (
            <div className="bg-green-50 border border-green-200 rounded-lg p-4">
              <p className="text-green-800 text-sm">
                {seleccionesParquizado.length} elementos configurados en esta etapa.
              </p>
            </div>
          )}
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {elementosParquizado.map((elemento) => (
            <div key={elemento.id} className="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
              ...
              <div className="space-y-3">
                {elemento.opciones.map((opcion) => {
                  const yaSeleccionado = seleccionesParquizado.some(s => s.opcionId === `${elemento.id}-${opcion.id}`);
        ...
        {seleccionesParquizado.length > 0 && (
          <div className="mt-8 bg-white rounded-xl border border-gray-200 shadow-sm p-6">
            ...
            <div className="space-y-3">
              {seleccionesParquizado.map((seleccion) => (
